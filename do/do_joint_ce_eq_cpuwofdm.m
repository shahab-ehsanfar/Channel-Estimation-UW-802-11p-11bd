function [ d_hat, H_hat_out, Rx_matrices ] = do_joint_ce_eq_cpuwofdm(config, snr, y, h_hat_primary, R_hh_err_primary, noblk, wiener_flag)
...
N = config.N; 
Ncp = config.Ncp;
UW_length_withcp = config.UW_length_withcp;
numTaps = config.numTaps; % number of channel taps
fd = config.fd; % maximum Doppler shift
Fs = config.Fs; % Sampling frequency
n_Hcpuw = config.n_Hcpuw; % sample index where CE is at its best
N_Nuwcp = config.N_Nuwcp; % N_Nuw = N (data) + Nuw = N (data) + N_preamble/2
UW_inFreq = config.UW_inFreq;
...
for blk = 1:noblk  
    ...
    if (wiener_flag ~= 2 || blk <= B)
        for iR = 1:nR           
%             [h_hat_secondary(:,:,iR), R_hh_err_2nd(:,:,iR)] = do_fft_channel_estimation( config, yp(:,iR), UW_inFreq, snr, 'true', 'exact LMMSE');
            h_hat_secondary(:,:,iR) = do_fft_channel_estimation( config, yp(:,iR), UW_inFreq, snr, 0, 'low complexity approx. LMMSE - UW');       
    ...
    if ( (wiener_flag == 1 && blk>1) || (wiener_flag == 2 && blk < B))
        ...
        for iR = 1:nR
            for iT = 1:nT
                ...
                [h_all_blks(:,iT,iR,:), Covariance(:,:,iT,iR,:)] = do_wiener_smoodiction_uw(h_hat_wiener_in, R_hh_err_2nd_iT, size(h_hat_wiener_in,2), N_Nuwcp, n_Hcpuw, n_Hd, Fs, fd, numTaps);
            end
        end
...
%         h_hat_out{blk+1} = h_all_blks(:,:,:,end-1);
    ...
    yd = do_emulate_cp(config,h_hat_in,y,blk,'for_data_cpUW');
